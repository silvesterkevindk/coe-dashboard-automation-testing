...
import { resourceDivision } from '../lib/division.js'

const alertDot = { red: '🔴', orange: '🟠', green: '🟢' }

// Divisi aktif dari sub-menu Dashboard
...
export default function Dashboard() {
  ...
  // Team Workload: bila divisi -> Test Lead + B2B divisi tsb; default -> semua resource
  const workloadResources = division
    ? resources.filter((r) => resourceDivision(r) === division)
    : resources
  // KPI Resource: bila divisi -> jumlah B2B divisi (Automation 21 / Performance 13); default -> total
  const b2bJabatan = division === 'Performance' ? 'B2B Performance' : 'B2B Automation'
  const resourceCount = division
    ? resources.filter((r) => (r.jabatan || '') === b2bJabatan).length
    : kpi.resourceCount
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <StatCard label="Resource" value={resourceCount} icon="👥" />
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3">
          ...
          {workloadResources.length === 0 && <p className="text-sm text-slate-400">Belum ada resource.</p>}
        </div>
